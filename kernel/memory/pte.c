...
void init_pte_info(void)
{
	...
	for (i = 0;i < KERNEL_SIZE / PTSIZE;i++)
	{
		...
		if (i > 0) pte_info[i].prev = &pte_info[i - 1]; else pte_info[i].prev = NULL;
		if (i < KERNEL_SIZE / PTSIZE - 1) pte_info[i].next = &pte_info[i + 1]; else pte_info[i].next = NULL;
	}
	for (i = KERNEL_SIZE / PTSIZE; i < PHY_MEM / PTSIZE;i++)
	{
		pte_info[i].physbase = i * PTSIZE;
		if (i > KERNEL_SIZE / PTSIZE) pte_info[i].prev = &pte_info[i - 1]; else pte_info[i].prev = NULL;
		if (i < PHY_MEM / PTSIZE - 1) pte_info[i].next = &pte_info[i + 1]; else pte_info[i].next = NULL;
	}
...
uint32_t get_pte()
{
	Assert(_free_pte != NULL, "No free pte left!\n");
